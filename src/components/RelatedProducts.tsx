...
const RelatedProducts = ({category, subCategory}: any) => {
  ...
  useEffect(() => {
    if (products.length > 0) {
      ...
      relatedProducts = relatedProducts.filter((item: Product) => category === item.category)
      relatedProducts = relatedProducts.filter((item: Product) => subCategory === item.subCategory)
